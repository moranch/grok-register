...
def _detect_mail_provider(api_base: str) -> str:
    if TEMP_MAIL_PROVIDER in {"duckmail", "temp-mail", "temp_mail", "generic", "tmail"}:
        if TEMP_MAIL_PROVIDER == "duckmail":
            return "duckmail"
        if TEMP_MAIL_PROVIDER == "tmail":
            return "tmail"
        return "generic"
    hostname = (urlparse(api_base).hostname or "").lower()
    if "duckmail" in hostname:
        return "duckmail"
    if "nnioj" in hostname or "tmail" in hostname:
        return "tmail"
    return "generic"
...
def _build_tmail_headers(api_key: str = "") -> Dict[str, str]:
    headers: Dict[str, str] = {}
    if api_key:
...
def _create_tmail_email() -> Tuple[str, str, str]:
    api_base = TEMP_MAIL_API_BASE.rstrip("/")
    api_key = TEMP_MAIL_ADMIN_PASSWORD
    session, use_cffi = _create_session()
    headers = _build_tmail_headers(api_key)
    ...
    if res.status_code != 200:
        raise Exception(f"创建 TMail 邮箱失败: {res.status_code} - {res.text[:200]}")

    data = res.json()
    if not isinstance(data, dict):
        raise Exception("TMail 创建邮箱接口返回格式异常")

    email = data.get("email") or ""
    if not email:
        raise Exception(f"TMail 创建邮箱未返回 email: {data}")

    print(f"[*] TMail 临时邮箱创建成功: {email}")
    return email, "", api_key


def _fetch_tmail_emails(mail_token: str, email: str = "") -> List[Dict[str, Any]]:
    api_base = TEMP_MAIL_API_BASE.rstrip("/")
    headers = _build_tmail_headers(mail_token)
    session, use_cffi = _create_session()
    ...
    if isinstance(data, dict):
        return data.get("messages") or data.get("data") or data.get("results") or []
    return []


def _fetch_tmail_email_detail(mail_token: str, msg_id: str) -> Optional[Dict[str, Any]]:
    api_base = TEMP_MAIL_API_BASE.rstrip("/")
    headers = _build_tmail_headers(mail_token)
    session, use_cffi = _create_session()
...
def create_temp_email() -> Tuple[str, str, str]:
    ...
    if provider == "tmail":
        if not TEMP_MAIL_ADMIN_PASSWORD:
            raise Exception("temp_mail_admin_password 未设置（TMail 需要 API Key）")
        try:
            return _create_tmail_email()
        except Exception as e:
            raise Exception(f"TMail 临时邮箱创建失败: {e}")
...
def fetch_emails(mail_token: str, email: str = "") -> List[Dict[str, Any]]:
    """获取邮件列表。"""
    provider = _detect_mail_provider(TEMP_MAIL_API_BASE)
    if provider == "duckmail":
        try:
    ...
    if provider == "tmail":
        try:
            return _fetch_tmail_emails(mail_token, email)
        except Exception:
            return []
...
def fetch_email_detail(mail_token: str, msg_id: str) -> Optional[Dict[str, Any]]:
    ...
    if provider == "tmail":
        try:
            return _fetch_tmail_email_detail(mail_token, msg_id)
        except Exception:
            return None
...
def wait_for_verification_code(mail_token: str, email: str = "", timeout: int = 120) -> Optional[str]:
    ...
    start = time.time()
    seen_ids = set()
    print(f"[*] 开始轮询验证码，邮箱: {email}, 超时: {timeout}秒")

    while time.time() - start < timeout:
        messages = fetch_emails(mail_token, email)
        print(f"[*] 轮询中... 获取到 {len(messages)} 封邮件")
        if messages:
        ...
        for msg in messages:
            ...
            # 优先从邮件列表的 subject 提取验证码（TMail 已包含）
            subject = msg.get("subject", "")
            if subject:
                code = extract_verification_code(subject)
                if code:
                    print(f"[*] 从 {_provider_label()} subject 提取到验证码: {code}")
                    return code
...
def _extract_mail_content(detail: Dict[str, Any]) -> str:
    """兼容 text/html/raw MIME/content 多种内容来源。"""
    # 优先使用 subject（TMail邮件列表已包含）
    subject = detail.get("subject", "")
    
    # 忽略TMail自动提取的code（可能不准确）
    # 只使用原始内容
    direct_parts = [
        subject,
        detail.get("text"),
        detail.get("html"),
        detail.get("content"),  # TMail 返回 content 字段
        detail.get("raw"),
        detail.get("source"),
    ]
    direct_content = "\n".join(_stringify_mail_part(part) for part in direct_parts if part)
    
    if subject or detail.get("text") or detail.get("html") or detail.get("content"):
