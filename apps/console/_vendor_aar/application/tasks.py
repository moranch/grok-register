...
from core._vendor_aar.base_platform import AccountStatus, RegisterConfig
from core._vendor_aar.datetime_utils import format_local_clock, serialize_datetime
from core._vendor_aar.db import AccountModel, TaskEventModel, TaskLog, TaskModel, engine, save_account
from core._vendor_aar.platform_accounts import build_platform_account
from core._vendor_aar.registry import get
...
def _auto_push_any2api(task_logger: TaskLogger, account) -> None:
    ...
    try:
        from core._vendor_aar.any2api_sync import push_account_to_any2api
...
def _auto_upload_cpa(task_logger: TaskLogger, account) -> None:
    if getattr(account, "platform", "") != "chatgpt":
        return
    try:
        from core._vendor_aar.config_store import config_store

        cpa_url = config_store.get("cpa_api_url", "")
        if cpa_url:
            from platforms._vendor_aar.chatgpt.cpa_upload import generate_token_json, upload_to_cpa
...
def _build_platform_instance(platform_name: str, payload: dict[str, Any], logger: TaskLogger, resolved_proxy: str | None = None, shared_mailbox=None):
    from core._vendor_aar.base_identity import normalize_identity_provider
    from core._vendor_aar.base_mailbox import create_mailbox
    ...
    if mailbox is None and identity_provider == "mailbox":
        if not extra.get("mail_provider"):
            from _vendor_aar.infrastructure.provider_settings_repository import ProviderSettingsRepository
...
def _resolve_sms_provider_for_task(extra: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    from _vendor_aar.infrastructure.provider_definitions_repository import ProviderDefinitionsRepository
    from _vendor_aar.infrastructure.provider_settings_repository import ProviderSettingsRepository
    ...
    try:
        ...
        def _hero_phone_alive() -> bool:
            if not (herosms_enabled and hero_reuse_to_max):
                return False
            try:
                from core._vendor_aar.base_sms import is_herosms_phone_cache_alive
