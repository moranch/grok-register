...
return true;
            ...
            if clicked:
                ...
                # 等待页面响应，检查是否跳转到验证码页面或显示错误
                time.sleep(3)
                # 检查页面是否有错误提示
                try:
                    error_text = page.run_js(r"""
                        const errorEl = document.querySelector('[data-testid="error"], .error, [role="alert"]');
                        return errorEl ? (errorEl.innerText || errorEl.textContent || '').trim() : '';
                    """)
                    if error_text:
                        print(f"[Warn] x.ai页面显示错误: {error_text}")
                        raise Exception(f"x.ai页面错误: {error_text}")
                except Exception as e:
                    if "x.ai页面错误" in str(e):
                        raise
...
def run_single_registration(output_path=DEFAULT_SSO_FILE, extract_numbers=False):
    ...
    try:
        ...
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        page.get_screenshot(path=os.path.join(debug_dir, f"after_email_{ts}.png"))
        html = page.run_js("return document.documentElement.outerHTML")
        with open(os.path.join(debug_dir, f"after_email_{ts}.html"), "w", encoding="utf-8") as f:
            f.write(html)
        print(f"[*] 已保存提交邮箱后的截图和HTML到 {debug_dir}")
    except Exception as e:
        print(f"[Warn] 保存调试信息失败: {e}")
